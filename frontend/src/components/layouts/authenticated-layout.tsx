...
<Button
  ...
  className={cn(
    "bg-transparent hover:bg-muted p-2 rounded-full transition-colors",
    "text-black dark:text-muted-foreground"
  )}
  type="button"
>
  {isCollapsed ? (
    <PanelRight className="h-5 w-5 transition-colors text-black dark:text-muted-foreground" />
  ) : (
    <PanelLeft className="h-5 w-5 transition-colors text-black dark:text-muted-foreground" />
  )}
</Button>
...
<Button
  ...
  className={cn(
    ...
    // Text color for light and dark themes
    "text-black dark:text-muted-foreground"
  )}
  type="button"
>
  <LogOut
    className={cn(
      ...
      // Icon color for light and dark themes
      "text-black dark:text-muted-foreground"
    )}
  />
  <span
    className={cn(
      "ml-2 truncate transition-colors",
      isCollapsed && "hidden",
      // Label color for light and dark themes
      "text-black dark:text-muted-foreground"
    )}
  >
    Logout
        ...
        <main className="flex flex-1 flex-col">
            {isMobile && (
              <header className="flex h-16 items-center border-b bg-background px-4 sticky top-0 z-30">
                <SheetTrigger>
                  <Button
                    className="bg-transparent hover:bg-muted p-2 rounded-full"
                    type="button"
                  >
