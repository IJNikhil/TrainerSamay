...
export default function UpcomingSessionNotification({
  ...
  const trainerName =
    currentTrainer?.role === "trainer"
      ? currentTrainer.name
      : trainers.find((t) => t.id === upcomingSession.trainerId)?.name || "Trainer";
  ...
  return (
    <Alert className="bg-accent/20 border-accent/50">
      ...
      <AlertDescription>
        {trainerName === currentTrainer?.name ? "Your" : `${trainerName}'s`} next session is a{" "}
        <strong>{upcomingSession.sessionType}</strong> class for the{" "}
        <strong>{upcomingSession.batch}</strong> batch on{" "}
        <strong>
          {format(sessionDate, "eeee, MMMM do")} at {format(sessionDate, "p")}
        </strong>.
      </AlertDescription>
    </Alert>
  );
