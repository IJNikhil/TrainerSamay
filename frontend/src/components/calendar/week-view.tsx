...
// import { ChevronLeft, ChevronRight, CheckCircle, Ban, AlertTriangle, Users, MapPin, CalendarDays, User as UserIcon } from 'lucide-react';

// interface WeekViewProps {
//   sessions: Session[];
//   onSessionClick: (session: Session) => void;
//   trainers?: User[]; // Optional with default in function params
// }
...
//                   <h3 className={cn("font-bold text-xl", isToday(day) ? 'text-primary' : '')}>{format(day, 'eeee')}</h3>
//                   <p className="text-muted-foreground text-sm">{format(day, 'MMMM d')}</p>
//                 </div>
//                 <div className="flex-1">
//                   {daySessions.length > 0 ? (
//                     <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
//                       {daySessions.map(session => {
//                         const trainer = trainers.find((t: User) => String(t.id) === String(session.trainerId));
//                         const sessionDate = session.date instanceof Date ? session.date : new Date(session.date);
//                         return (
//                           <button
//                             key={session.id}
...
//                             className={cn(
//                               'w-full text-left p-3 rounded-lg text-sm transition-all duration-200 border-l-4 flex flex-col justify-between min-h-[140px] hover:shadow-lg hover:-translate-y-0.5',
...
//                             )}
//                           >
//                             <div className={cn({ 'text-muted-foreground': session.status !== 'Scheduled' })}>
//                               <div className="flex items-start justify-between gap-1">
//                                 <p className="font-bold text-base text-foreground truncate">{session.sessionType}</p>
...
//       <CheckCircle className="h-4 w-4 mr-1" /> Started
//     </span>
//   )}
...
//       <Ban className="h-4 w-4 mr-1" /> Cancelled
//     </span>
//   )}
//   {session.status === 'Absent' && (
//     <span className="flex items-center gap-1 bg-amber-100 text-amber-800 dark:bg-amber-900/50 dark:text-amber-200 border border-amber-300 dark:border-amber-700 rounded-full px-2 py-0.5 text-xs font-semibold">
//       <AlertTriangle className="h-4 w-4 mr-1" /> Absent
//     </span>
//   )}
// </div>
...
//                               <p className="flex items-center gap-1.5 truncate text-xs mt-1 text-current"><UserIcon className="h-3.5 w-3.5" /> {trainer?.name || 'N/A'}</p>
//                             </div>
//                             <div className={cn("mt-2 space-y-1", { 'text-muted-foreground': session.status !== 'Scheduled' })}>
//                               <p className="flex items-center gap-1.5 truncate text-xs"><Users className="h-3.5 w-3.5" /> {session.batch}</p>
//                               <p className="flex items-center gap-1.5 truncate text-xs"><MapPin className="h-3.5 w-3.5" /> {session.location}</p>
//                             </div>
//                           </button>
//                         );
//                       })}
...
// Removed sessionTypeBorderClasses
